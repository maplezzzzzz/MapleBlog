...
const EnhancedMarkdownEditor = ({ value, onChange, inputId, ...props }) => {
        ...
        commands={[
          ...
          // 自定义图片上传命令
          {
            ...
            render: (command, disabled, executeCommand) => {
              return (
                <div className="w-md-editor-command w-md-editor-command-image">
                  <input
                    type="file"
                    accept="image/*"
                    style={{ display: 'none' }}
                    ...
                    onChange={async (e) => {
                      ...
                      if (file) {
                        const url = await customUploadImage(file);
                        if (url) {
                          // 插入图片到编辑器
          ...
          MDEditor.commands.unorderedlist,
          MDEditor.commands.orderedlist,
          MDEditor.commands.checked,
        ]}
      />
    </div>
  );
