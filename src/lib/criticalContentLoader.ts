...
export class CriticalContentLoader {
  ...
  private identifyCriticalElements(): void {
    ...
    const criticalMedia = Array.from(
      document.querySelectorAll<HTMLElement>('img[data-critical="true"], iframe[data-critical="true"]')
   ...
   * 检查元素是否在视口内
   */
  private isElementInViewport(element: HTMLElement, viewportHeight: number): boolean {
  ...
  private setupLazyLoading(): void {
    ...
    this.nonCriticalElements = Array.from(
      document.querySelectorAll<HTMLElement>('[data-non-critical], img[data-src]:not([data-critical="true"])')
