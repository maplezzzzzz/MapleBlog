...
function validateOptimizations() {
  ...
  // 检查关键类是否存在
  const checks = [
    { name: 'BrowserHistoryManager', exists: typeof BrowserHistoryManager !== 'undefined' },
    { name: 'PageTransitionManager', exists: typeof PageTransitionManager !== 'undefined' },
    { name: 'CriticalContentLoader', exists: typeof CriticalContentLoader !== 'undefined' },
    { name: 'SmartResourcePreloader', exists: typeof SmartResourcePreloader !== 'undefined' }
  ];
  ...
  if ('performance' in window) {
    ...
    window.addEventListener('load', () => {
      setTimeout(() => {
        ...
        const avgResourceLoadTime = resources.reduce((sum, res) => sum + res.duration, 0) / resources.length;
        console.log(`⏱️ 平均资源加载时间: ${avgResourceLoadTime ? avgResourceLoadTime.toFixed(2) + 'ms' : 'N/A'}`);
      }, 5000); // 延迟5秒以确保资源加载完成
    });
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setInterval(() => {
    ...
    const activeStyles = document.querySelectorAll('link[rel="stylesheet"]').length;
